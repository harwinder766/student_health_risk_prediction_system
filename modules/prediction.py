...
def load_input_options():
    with open(BASE_DIR / "config" / "input_options.json") as f:
        return json.load(f)


@st.cache_data
def load_sleep_probe():
    return joblib.load(BASE_DIR / "models" / "sleep_probe_1.pkl")


@st.cache_data
def load_stress_probe():
    return joblib.load(BASE_DIR / "models" / "stress_probe_1.pkl")


@st.cache_data
def load_weights():
    return np.load(BASE_DIR / "models" / "best_weights_catboost.npy")


@st.cache_resource
def load_artifacts():
    return joblib.load(BASE_DIR / "models" / "final_catboost.pkl")
