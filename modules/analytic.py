...
def relationship_plot(df, x, y, numerical_cols, categorical_cols):
    ...
    if x in numerical_cols and y in numerical_cols:
        ...
        fig = px.box(plot_data,x=x, y=y,color="health_condition",
        ...
        fig = px.box(plot_data, x=y, y=x, color="health_condition",
...
@st.cache_data
def load_df():
    df = pd.read_parquet(BASE_DIR / "data" / "analytics_data.parquet")
    return df
...
# Initialize numerical filter states
...
if "exercise_duration" not in st.session_state:
    st.session_state.exercise_duration = (
...
if "calorie_expenditure" not in st.session_state:
    st.session_state.calorie_expenditure = (
...
heart_rate = st.sidebar.slider(
    "Heart Rate",
    min_value=float(df["heart_rate"].min()),
    max_value=float(df["heart_rate"].max()),
    key="heart_rate"
)
...
calorie_expenditure = st.sidebar.slider(
    "Calorie Expenditure",
    min_value=int(df["calorie_expenditure"].min()),
    max_value=int(df["calorie_expenditure"].max()),
    key="calorie_expenditure"
)

step_count = st.sidebar.slider(
    "Step Count",
    min_value=int(df["step_count"].min()),
    max_value=int(df["step_count"].max()),
    key="step_count"
)

exercise_duration = st.sidebar.slider(
    "Exercise Duration (in minutes)",
    min_value=float(df["exercise_duration"].min()),
    max_value=float(df["exercise_duration"].max()),
    key="exercise_duration"
)

water_intake = st.sidebar.slider(
    "Water Intake",
    min_value=float(df["water_intake"].min()),
    max_value=float(df["water_intake"].max()),
    key="water_intake"
)
